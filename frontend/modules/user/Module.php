...
class Module extends \yii\base\Module
{
    ...
    /**
     * @var array Model classes, e.g., ["User" => "frontend\modules\user\models\User"]
     * Usage:
     *   $user = Yii::$app->getModule("user")->model("User", $config);
     *   (equivalent to)
     *   $user = new \frontend\modules\user\models\User($config);
     *
    ...
    protected function checkModuleProperties()
    {
        ...
        if ($userComponent && !$userComponent instanceof \frontend\modules\user\components\User) {
            throw new InvalidConfigException('Yii::$app->user is not set properly. It needs to extend \frontend\modules\user\components\User');
        }
    }
    ...
    protected function getDefaultModelClasses()
    {
        ...
        if (Yii::$app->get('user', false)) {
            ...
            $userClass = 'frontend\modules\user\models\User';
        }

        return [
            'User' => $userClass,
            'Profile' => 'frontend\modules\user\models\Profile',
            'Role' => 'frontend\modules\user\models\Role',
            'UserToken' => 'frontend\modules\user\models\UserToken',
            'UserAuth' => 'frontend\modules\user\models\UserAuth',
            'ForgotForm' => 'frontend\modules\user\models\forms\ForgotForm',
            'LoginForm' => 'frontend\modules\user\models\forms\LoginForm',
            'ResendForm' => 'frontend\modules\user\models\forms\ResendForm',
            'UserSearch' => 'frontend\modules\user\models\search\UserSearch',
            'LoginEmailForm' => 'frontend\modules\user\models\forms\LoginEmailForm',
        ];
    }
    ...
    {
        // check valid routes
        $validRoutes  = [$this->defaultRoute, "admin", "copy", "auth","permission"];
        $isValidRoute = false;
